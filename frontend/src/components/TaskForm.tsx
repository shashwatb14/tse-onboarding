...
export function TaskForm({ mode, task, onSubmit }: TaskFormProps) {
  ...
  const [assignee, setAssignee] = useState<string>(task?.assignee?._id || ""); // New state variable for assignee
  ...
  const handleSubmit = () => {
    ...
    const taskData = { title, description, assignee: assignee || "Not assigned" };
      ...
      createTask(taskData)
        .then((result) => {
          if (result.success) {
            ...
            setTitle("");
            setDescription("");
            setAssignee(""); // Clear the assignee field
